...
adds `patches/<package>/*.patch`, appends `.pkcs11` to the Release, and builds. On Fedora
...
The clones are not part of this repository. To recreate them, clone upstream, check out
the `BASE` tag as branch `pkcs11`, and apply the patches with `git am`:

```sh
git clone https://gitlab.gnome.org/GNOME/NetworkManager-openvpn.git src/NetworkManager-openvpn
git clone https://invent.kde.org/plasma/plasma-nm.git src/plasma-nm
for p in NetworkManager-openvpn plasma-nm; do
	git -C src/$p checkout -b pkcs11 "$(cat patches/$p/BASE)"
	git -C src/$p am "$PWD"/patches/$p/*.patch
done
```
...
scripts/                    rebuild, build-rpm.sh, install, copr-publish, export-patches, ...
tests/                      see above
